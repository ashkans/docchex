...
@duty
def eval(ctx: Context) -> None:
    """Run the evaluation benchmark suite."""
    ctx.run(
        ["uv", "run", "python", "evals/runner.py"],
        title="Running eval suite",
        pty=PTY,
        capture=False,
    )
...
def coverage(ctx: Context) -> None:
    ...
    ctx.run(
        tools.coverage.json(rcfile="config/coverage.ini", output="reports/coverage.json"),
        title="Exporting coverage to reports/coverage.json",
    )
